...
### Notifications
Three layers, all driven by `NotificationsProvider` (`src/components/NotificationsProvider.tsx`, mounted in `AppShell`):
- **Unread state**: chat uses `ChannelMember.lastReadAt` (no per-message rows — `getUnreadSummary()` in `src/lib/notifications.ts` counts others' messages after it); task assignments are `Notification` rows (`notifyTaskAssigned()`, called from task create/PATCH when the assignee changes, never for self-assignment). `GET /api/notifications` returns both; `POST /api/channels/[channelId]/read` and `POST /api/notifications/read` clear them. The provider marks things read when you look at them (open chat, `/my-tasks`, a task opened on a board).
- **Live alerts**: `GET /api/notifications/stream` is an app-wide polling SSE (same shape as the channel stream) sending new messages from the user's channels and new task notices. A visible tab shows a toast; a hidden tab shows a system notification via the service worker — unless this device has Web Push, which then handles it. One tab per browser (a Web Locks holder) raises system notifications.
- **Web Push**: `public/sw.js` (notification-only service worker, no caching) + `web-push` (`src/lib/push.ts`, off unless `NEXT_PUBLIC_VAPID_PUBLIC_KEY`/`VAPID_PRIVATE_KEY`/`VAPID_SUBJECT` are set). `PushSubscription` rows belong to the Auth.js `Session` they were registered under, so signing out (session row deleted) stops pushes to that browser; `POST /api/push/subscription` reads the session cookie off the request to link them. Push text is rendered server-side in the subscription's stored locale via `src/lib/notificationText.ts`, which the client uses too — keep them in sync there. iOS only allows push for the installed app (`src/app/manifest.ts`, icons from `src/app/pwa-icon/[variant]`); the SW skips showing a push while an app tab is visible.
