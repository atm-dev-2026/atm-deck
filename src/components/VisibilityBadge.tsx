...
const config: Record<
  BoardVisibility,
  { labelKey: "global" | "department" | "personal"; icon: typeof Globe2; className: string }
> = {
  GLOBAL: {
    labelKey: "global",
    icon: Globe2,
...
export function VisibilityBadge({ visibilityType }: { visibilityType: BoardVisibility }) {
  const t = useTranslations("Boards.visibility");
  const { labelKey, icon: Icon, className } = config[visibilityType];
  return (
    <span
